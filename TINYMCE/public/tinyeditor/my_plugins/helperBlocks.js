    ...
    onSubmit(api) {
      ...
      if (data.title) {
        switch (data.hideshow) {
          case 'show':
            let textShow = `<div class="${data.color}"><h4>${data.title}</h4>`;
          ...
          case 'hide':
            ...
            let textHide = `<a class="button green showHideHelperInfo" style="text-align:center;width:100%" data-show="${divid}">`;
            textHide += `${data.title}</a><div id="${divid}" class="${data.color} HelperInfoHidden">`;
            textHide += `<h3>${data.title}</h3><p>${data.text}</p><br><br></div><br><br>`;
